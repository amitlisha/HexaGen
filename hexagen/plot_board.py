...
def plot_boards(boards, fig_size=(5, 5), width=18, height=10, max_in_row=6, h_pad=0, edge_color='k',
                color_map='hexa', titles=None, show=True):
    '''Plot a hexagon board / boards'''
...
def make_grid(nx, ny, min_diam, n, crop_circ, rotate_deg, align_to_origin) -> (np.ndarray, np.ndarray):
    ...
    if not np.isclose(rotate_deg, 0):
        rot_matrix = np.array([[np.cos(np.deg2rad(rotate_deg)), np.sin(np.deg2rad(rotate_deg))],
...
def plot_single_lattice(fig_size, coord_x, coord_y, face_color, edge_color, min_diam, plotting_gap, rotate_deg,
    ...
    for curr_x, curr_y in zip(coord_x.ravel(), coord_y.ravel()):
        polygon = mpatches.RegularPolygon((float(curr_x), float(curr_y)), numVertices=6,
    ...
    h_ax.axis([coord_x.min() - 2 * min_diam, coord_x.max() + 2 * min_diam,
               coord_y.min() - 2 * min_diam, coord_y.max() + 2 * min_diam])
