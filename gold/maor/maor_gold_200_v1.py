...
'''
line = Line(start_tile=Tile(column=3, row=-1), end_tile=Tile(column=3, row=-2))
line.draw('purple')
...
'''
for i in range(7):
  line = line.parallel(shift_direction='right', spacing=1)
  
  column = line.start_tile.column 
  red_in_column = Shape.get_column(column) * Shape.get_color('red')
  tile = red_in_column.edge(direction='down')
  upper_line = Line(start_tile=tile, direction='up')
  
  (line-upper_line).draw('purple')
