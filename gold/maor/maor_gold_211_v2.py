...
'''
HexagonsGame.record_step(step_name='flower')
...
'''
HexagonsGame.record_step(step_name='triangle')
...
'''
HexagonsGame.record_step(step_name='row')

flower = HexagonsGame.get_record(step_names=['flower'])
triangle = HexagonsGame.get_record(step_names=['triangle'])

for i in range(3):
  flower = flower.copy_paste(shift_direction='right', spacing=3)
  triangle = triangle.copy_paste(shift_direction='right', spacing=3)

HexagonsGame.record_step(step_name='row_end')
'''
...
the one above it.
'''
row = HexagonsGame.get_record(step_names=['flower', 'triangle', 'row'])
