...
for i in range(4):
  ...
  tile.draw('purple')
HexagonsGame.record_step('1_end')
...
for i in range(4):
  ...
  tile.draw('blue')
HexagonsGame.record_step('2_end')
'''
...
the leftmost column.
'''
pattern = HexagonsGame.get_record(['1'])
r = pattern.edge('top').row
for c in [-2, -4, -6, -10]:
  pattern.copy_paste(source=pattern.edge('top'), destination=Tile(c,r))
...
'''
pattern = HexagonsGame.get_record(['2'])
r = pattern.edge('top').row
for c in [6, 8, 16, -1]:
  pattern.copy_paste(source=pattern.edge('top'), destination=Tile(c,r))
...
'''
pattern = HexagonsGame.get_record(['4'])
r = pattern.edge('top').row
for c in [-5, -7, -9]:
  pattern.copy_paste(source=pattern.edge('top'), destination=Tile(c,r))
