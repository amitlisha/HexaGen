...
- Stage 2: Implementation generation
- Stage 3: DSL documentation generation
"""
...
def main():
    ...
    if "2" in stages_to_run:
        ...
        if 1 in results:
            ...
            # Use Stage 1 API directly
            stage1_result = {
                "stage": 1,
                "outputs": {"final_api": results[1]["outputs"]["api_proposal"]},
            }

        # Run Stage 2 if we have input (from Stage 1)
        if stage1_result:
            from stage2_implementation import run_stage2

            stage2_dir = exp_dir / "stage2"
            stage2_dir.mkdir(parents=True, exist_ok=True)
            results[2] = run_stage2(cfg, stage2_dir, stage1_result)

    # Stage 3: Documentation
    if "3" in stages_to_run:
        # Check if Stage 2 results exist (either from current run or from disk)
        if 2 not in results:
        ...
        if 2 in results:
            from stage3_documentation import run_stage3

            stage3_dir = exp_dir / "stage3"
            stage3_dir.mkdir(parents=True, exist_ok=True)
            results[3] = run_stage3(cfg, stage3_dir, results[2])
