...
Usage
-----
$ python experiment.py --task 24 --model gpt-4o
"""
...
def run_step(cfg: argparse.Namespace, sys_prompt: str, user_tmpl: str,
             ...
             out_dir: Path, step_idx: int, run_ts: str) -> tuple[Dict, bool, str]:
    """
    ...
    while True:
        ...
        try:
            ...
            save_plot(pred, gold_board,
                    out_dir / f"{run_ts}_plot_{step_idx:02}_{attempt:02}.png")
            return log, True, new_script
        except Exception:
            log["traceback"] = traceback.format_exc()
            if cfg.retries and attempt >= cfg.retries:
                return log, False, code

            
def run_full(cfg, sys_prompt, user_tmpl, instructions,
             code_so_far, gold_final, task_dir, run_ts):
    """
...
def main() -> None:
    ...
    if cfg.mode == "step":
        ...
        log = run_full(cfg, sys_prompt, user_tmpl,
                    instructions, code_so_far,
                    gold_boards[-1], out_dir, run_ts)
        logs = [log]
        print("FULL RUN →",
