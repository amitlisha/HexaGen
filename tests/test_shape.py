...
def test_shape_grid(game):
    ...
    assert_shape_linds(
        Shape([56, 37, 38], from_linds=True).grid(
            shift_direction="right", spacing=2, num_copies=2
        ),
...
def test_copy_paste(game):
    assert_shape_linds(
        Shape([60, 41, 42], from_linds=True).copy_paste(
            shift_direction="right", spacing=3
        ),
        [65, 64, 47],
    )
    assert_shape_linds(
        Shape([60, 41, 42], from_linds=True).copy_paste(
            shift_direction="down_right", spacing=2
        ),
        [81, 82, 100],
    )
    assert_shape_linds(
        Shape([60, 41, 42], from_linds=True).copy_paste(
            shift_direction="right", spacing=2, reference_shape=Tile(5, 10)
        ),
        [85, 84, 67],
    )
...
def test_reflect(game):
    assert_shape_linds(
        Shape([40, 41, 42], from_linds=True).reflect(column=10), [48, 49, 50]
    )
    assert_shape_linds(
        Shape([40, 41, 42], from_linds=True).reflect(
            axis_line=Line(Tile(5, 7), direction="down_left")
        ),
        [97, 98, 115],
    )
    assert_shape_linds(
        Shape([40, 41, 42], from_linds=True).reflect(
            axis_direction="down_left", tile_on_axis=Tile(5, 7)
        ),
        [97, 98, 115],
    )
...
def test_rotate(game):
    assert_shape_linds(
        Shape([40, 41, 42], from_linds=True).rotate(Tile(5, 9), 120), [152, 115, 134]
    )
...
def test_get_column(game):
    assert_shape_linds(
        Shape.get_column(column=1), [0, 18, 36, 54, 72, 90, 108, 126, 144, 162]
    )
...
def test_get_criterion(game):
    ...
    assert_shape_linds(
        S.get(criterion="above"), [7, 8, 9, 10, 43, 44, 11, 46, 47, 25, 26, 27, 28, 29]
    )
    assert_shape_linds(
...
def test_get_invalid_criterion(game):
